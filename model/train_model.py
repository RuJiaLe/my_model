...
from .utils import Video_Encoder_Part, self_attention
...
class Video_Decoder_Model(nn.Module):
    def __init__(self):
        ...
        # --------------------第四解码阶段--------------------
        self.decoder4 = self_attention(1024, 512)
        # --------------------第三解码阶段--------------------
        ...
        self.decoder2 = self_attention(256, 128)
    ...
    def forward(self, blocks):  # (1, 1024, 16, 16)
        ...
        state = None
        state = self.ConvLSTM3_1(x3_4, state)
        out3_4 = state[0]
        state = self.ConvLSTM3_2(x3_3, state)
        out3_3 = state[0]
        state = self.ConvLSTM3_3(x3_2, state)
        out3_2 = state[0]
        state = self.ConvLSTM3_4(x3_1, state)
        out3_1 = state[0]

        out3_1 = self.Up_sample_2(out3_1)  # (1, 256, 64, 64)
        out3_2 = self.Up_sample_2(out3_2)
        out3_3 = self.Up_sample_2(out3_3)
        out3_4 = self.Up_sample_2(out3_4)
        ...
        out1_1 = self.Up_sample_2(out1_1)  # (1, 64, 256,256)
        out1_2 = self.Up_sample_2(out1_2)
        out1_3 = self.Up_sample_2(out1_3)
        out1_4 = self.Up_sample_2(out1_4)
