...
def start_video_train(video_train_dataloader, video_val_dataloader, video_encoder_model, video_decoder_model,
                      total_epoch, lr, decay_rate, decay_epoch, encoder_model_path, decoder_model_path, log_dir):
    # log
...
def start_image_train(image_train_dataloader, image_val_dataloader, image_encoder_model, image_decoder_model,
    ...
    if os.path.exists(encoder_model_path):
        ...
        image_encoder_model.load_state_dict(torch.load(encoder_model_path, map_location=torch.device(device)))

    if os.path.exists(decoder_model_path):
        print('Loading decoder_model from: {}'.format(decoder_model_path))
        image_decoder_model.load_state_dict(torch.load(decoder_model_path, map_location=torch.device(device)))
